...
const ContactsView = () => {
  ...
  const [selectedGroup, setSelectedGroup] = useState<string>(() => {
    ...
    // Default to "Constellations (Groups)" if nothing is saved
    return savedView || "Constellations (Groups)";
  });
  ...
  const [zoomLevel, setZoomLevel] = useState(50); // 0 = zoomed out (contacts close together), 100 = zoomed in (contacts far apart)
  ...
  const renderConstellationsView = () => {
    ...
    const renderGroupNode = (group: any, position: { x: number, y: number }) => {
      ...
      const zoomSizeFactor = 0.8 + (zoomLevel / 200); // 0.8 at zoom=0, 1.3 at zoom=100 (no change needed as the math works the same)
        ...
        >
          ...
          <div 
            ...
            style={{
              ...
              opacity: zoomLevel >= 50 ? 1 : 0.7, // More visible when zoomed in (closer to center) - still works with reversed polarity
  ...
  const renderOrbitsView = () => {
    ...
    // Also apply the search filter to match the behavior in other views
    ...
    // Calculate which orbits to show based on zoom level
    // When slider shows "zoomed in" (+), show all orbits (contacts farther from center)
    // When slider shows "zoomed out" (-), show all orbits (all contacts visible but closer together)
    const showInnerOrbit = true; // Always show inner orbit
    const showMiddleOrbit = true; // Always show middle orbit
    const showOuterOrbit = true; // Always show outer orbit

    // Calculate orbit radii based on zoom level
    // Base values represent the minimum distances when fully zoomed in
    const baseInnerRadius = 100;
    const baseMiddleRadius = 200;
    const baseOuterRadius = 300;
    
    // Reverse the zoom level to match the expected behavior
    // When slider shows "zoomed in" (100), contacts should be farthest from center
    // When slider shows "zoomed out" (0), contacts should be closest to center
    const reversedZoomLevel = zoomLevel; // No need to reverse since we want zoomed in = far away
    
    // Zoom factor: larger when slider shows "zoomed in" (100), smaller when slider shows "zoomed out" (0)
    // Adjusted to ensure all contacts are visible when zoomed out
    const zoomFactor = 0.3 + (reversedZoomLevel / 40); // 0.3 at zoom=0, 2.8 at zoom=100
    ...
    return (
      <div className="relative w-full h-full overflow-hidden">
        ...
        <div className="absolute right-6 top-1/2 transform -translate-y-1/2 z-50 h-64 flex flex-col items-center justify-between">
          <div className="text-purple-300 mb-2 bg-black/30 p-2 rounded-full hover:bg-black/50 cursor-pointer" onClick={() => setZoomLevel(Math.min(100, zoomLevel + 10))}>
          ...
          <div className="text-purple-300 mt-2 bg-black/30 p-2 rounded-full hover:bg-black/50 cursor-pointer" onClick={() => setZoomLevel(Math.max(0, zoomLevel - 10))}>
          ...
          {showMiddleOrbit && middleOrbitContacts.map((contact, index) => {
            ...
            return (
              ...
              >
                ...
                {zoomLevel > 30 && ( // Show names when zoomed in more than 30%
  ...
  const isDefaultGroup = selectedGroup === "Orbits (People)" || selectedGroup === "Deep Space";
  const renderGroupHeader = () => {
    if (selectedGroup === "Orbits (People)" || selectedGroup === "Deep Space") {
      return null;
    }
  ...
  const handleDeleteGroup = async () => {
    ...
    try {
      ...
      // Reset view to Constellations and refresh data
      setSelectedGroup("Constellations (Groups)");
    ...
    >
      <div className="relative flex flex-col h-full">
        ...
        <div className="flex flex-col gap-2 w-full mb-6">
          {/* Top row with Constellations and Orbits */}
            ...
            <div className="flex items-center justify-between gap-2 mb-4">
              <div className="flex items-center gap-2">
                ...
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="h-6 w-6 bg-purple-900/50 border-purple-500/50 text-white hover:bg-purple-800/50 ml-2" 
                  onClick={() => setIsGroupDialogOpen(true)}
                >
                  <Plus className="h-4 w-4" />
                </Button>
              ...
              <div className="flex items-center gap-2">
                <Badge 
                  variant="outline" 
                  className="cursor-pointer hover:bg-purple-800/50 text-sm bg-purple-900/50 border-purple-500/50 text-purple-100" 
                  onClick={() => setShowDeepSpace(true)}
                >
                  🌌 Deep Space (Unsorted Contacts)
                </Badge>
              </div>
            </div>
            {isGroupsExpanded && (
